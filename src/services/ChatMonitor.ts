...
import YouTubeService from '../features/auth';
 ...
 */
export default class ChatMonitor extends EventEmitter {
  ...
  _saveIntervalId!: ReturnType<typeof setInterval> | null;
  _statsEmitTimer!: ReturnType<typeof setInterval> | null;
  _processorTimer!: ReturnType<typeof setTimeout> | null;
  _pollInFlight!: boolean;
  _lastPollDelayMs!: number;
  _lastStatsHash!: string;

  messageDatabase!: Array<Record<string, unknown>>;
  analysisDatabase!: Array<Record<string, unknown>>;
  actionLog!: Array<Record<string, unknown>>;
  liveCommentLog!: Array<Record<string, unknown>>;
  smartCache!: Map<string, Record<string, unknown>>;
  userProfiles!: Map<string, Record<string, unknown>>;

  fastQueue!: Array<Record<string, unknown>>;
  aiPool!: AIWorkerPool<Record<string, unknown>> | null;
  ...
  smartRules!: { critical: RegExp[]; suspicious: RegExp[] };
   ...
   */
  async start(options: { videoId?: string } = {}) {
    if (this.isRunning) {
