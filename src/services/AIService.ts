...
import {
  createModerationGraph,
  type ModerationGraph
} from './ModerationGraph';
import { httpAgent, httpsAgent, attachToAxios } from './HttpAgent';
...
async function streamChatCompletionUntilVerdict({
  ...
  return await new Promise<{
    ...
    const finish = (text: string, earlyVerdict: boolean | null) => {
      if (resolved) return;
      resolved = true;
      clearTimeout(timer);
    ...
    stream.on('data', (chunk) => {
      ...
      for (const rawLine of lines) {
        ...
        const choice0 =
          choices && typeof choices[0] === 'object' && choices[0] !== null
            ? (choices[0] as Record<string, unknown>)
            : undefined;
        const deltaRaw = choice0?.delta ?? choice0?.message;
        const delta =
          (typeof deltaRaw === 'object' &&
            deltaRaw &&
            typeof (deltaRaw as { content?: string }).content === 'string'
            ? (deltaRaw as { content?: string }).content
            : '') ?? '';
        if (delta) buf += delta;
...
export default class AIService {
  ...
  private _moderationGraph: ModerationGraph | null = null;
  ...
  constructor(config: ConfigManager | null = null) {
    this.config = config;
  ...
  _cfg(path: string, fallback?: unknown): unknown {
  ...
  async analyze(message, author, context, settings) {
    ...
    if (this._streamingEnabled()) {
      try {
        ...
        console.warn(
          `LMStudio streaming failed (${e instanceof Error ? e.message : String(e)}); falling back to non-stream`
        );
      }
    }
  ...
  async analyze(message, author, context, settings) {
    ...
    if (this._streamingEnabled()) {
      try {
        ...
        console.warn(
          `OpenAI streaming failed (${e instanceof Error ? e.message : String(e)}); falling back to non-stream`
        );
      }
    }
