...
const MODE_SAFE = 'safe' as const;
const MODE_PLAIN = 'plaintext' as const;

export type SecretStoreMode = typeof MODE_SAFE | typeof MODE_PLAIN;
...
export default class SecretStore {
  ...
  private _safeStorage: SafeStorageApi | null = null;
  private _mode: SecretStoreMode = MODE_PLAIN;
   ...
   */
  async init(): Promise<SecretStoreMode> {
  ...
  private _tryLoadSafeStorage(): SafeStorageApi | null {
    try {
      ...
      const electron = require('electron') as Record<string, unknown> | string;
      if (!electron || typeof electron !== 'object') return null;
      const ss = electron.safeStorage as SafeStorageApi | undefined;
  ...
  private async _load(): Promise<void> {
    ...
    if (looksLikeJson) {
      try {
        this._cache = JSON.parse(raw.toString('utf8')) as Record<string, unknown>;
      } catch {
        console.warn(
    ...
    try {
      ...
      this._cache = JSON.parse(txt) as Record<string, unknown>;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(
        `SecretStore: failed to decrypt ${this.filePath}: ${msg}. ` +
  ...
  describe(): { mode: SecretStoreMode; filePath: string; keys: string[] } {
    return {
      mode: this._mode,
