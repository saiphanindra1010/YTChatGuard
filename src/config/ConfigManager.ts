...
import { assertSafeLocalUrl } from './UrlAllowlist';
...
interface BuiltinOAuth {
  clientId: string;
  clientSecret: string;
}

function loadBuiltinYoutubeOAuth(): BuiltinOAuth {
  try {
    const m = require('./builtin.youtube.oauth') as BuiltinOAuth;
...
class ConfigManager {
  ...
  set(pathKey: string, value: unknown): void {
    ...
    if (SECRET_PATHS.includes(pathKey)) {
      ...
      this.secrets.save().catch((err: unknown) => {
        console.error(
          'SecretStore save failed:',
  ...
  getModerationSettingsSnapshot() {
    const rs = (this.get('moderation.responses') || {}) as Record<
      string,
      unknown
    >;
    return {
   ...
   */
  watch(callback: (snapshot: Record<string, unknown>) => void): () => boolean {
  ...
  _validateConfig(): void {
    ...
    if (!this.get('youtube.clientId') || !this.get('youtube.clientSecret')) {
      console.warn(
        'YouTube OAuth not configured: add Client ID and Secret in System → Google OAuth (or builtin.youtube.oauth.ts).'
      );
    }
    ...
    for (const key in source) {
      ...
      if (sk && typeof sk === 'object' && !Array.isArray(sk)) {
        const base = (target[key] && typeof target[key] === 'object' && !Array.isArray(target[key])
          ? target[key]
          : {}) as Record<string, unknown>;
        result[key] = this._deepMerge(base, sk as Record<string, unknown>);
      } else {
        result[key] = sk;
      }
    }
   ...
   */
  _getNestedValue(obj: unknown, pathKey: string): unknown {
   ...
   */
  _setNestedValue(obj: Record<string, unknown>, pathKey: string, value: unknown): void {
