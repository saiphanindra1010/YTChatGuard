...
import YouTubeService from './src/features/auth';
import ChatMonitor from './src/services/ChatMonitor';
...
class SafeStream extends EventEmitter {
  ...
  youtubeService: YouTubeService | null = null;
  smartMonitor: ChatMonitor | null = null;
  ...
  getListenPort(): number {
    if (this._listenPort != null) return this._listenPort;
    const raw =
      this.config.get('server.port') ??
      this.config.get('app.port') ??
      3000;
  ...
  _isPublicHttpRoute(req: Request): boolean {
    const p = req.path || '';
  ...
  _apiAuthMiddleware(req: Request, res: Response, next: NextFunction): void {
