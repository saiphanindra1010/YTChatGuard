...
// @ts-nocheck — Parsed JSON blobs from LM Studio/OpenAI/Google until shared response schemas exist.
const axios_1 = __importDefault(require("axios"));
const generative_ai_1 = require("@google/generative-ai");
const ModerationGraph_1 = require("./ModerationGraph");
const HttpAgent_1 = require("./HttpAgent");
(0, HttpAgent_1.attachToAxios)(axios_1.default);
/**
 ...
 */
async function streamChatCompletionUntilVerdict({ url, model, messages, apiKey = null, maxTokens = 60, temperature = 0, timeoutMs = 8000 }) {
    ...
    try {
        response = await axios_1.default.post(url, {
            ...
            headers,
            responseType: 'stream',
            httpAgent: HttpAgent_1.httpAgent,
            httpsAgent: HttpAgent_1.httpsAgent,
    ...
    return await new Promise((resolve, reject) => {
        ...
        stream.on('data', (chunk) => {
            ...
            for (const rawLine of lines) {
                ...
                const choice0 = choices && typeof choices[0] === 'object' && choices[0] !== null
                    ? choices[0]
                    : undefined;
                const deltaRaw = choice0?.delta ?? choice0?.message;
                const delta = (typeof deltaRaw === 'object' &&
                    deltaRaw &&
                    typeof deltaRaw.content === 'string'
                    ? deltaRaw.content
                    : '') ?? '';
                if (delta)
                    buf += delta;
...
class AIService {
    ...
    _syncModerationSettings() {
        ...
        this.moderationSettings = {
            ...
            customRules: [],
            responseTemplates: {
                warning: this._cfg('moderation.responses.warning') ||
                    '{author} ⚠️ Please keep the chat respectful. This is your warning.',
                timeout: this._cfg('moderation.responses.timeout') ||
                    '{author} 🚫 Your message violated community guidelines. Timeout applied.',
                ban: this._cfg('moderation.responses.ban') ||
    ...
    async _analyzeViaLangGraph(message, author, context) {
        ...
        if (!this._moderationGraph) {
            this._moderationGraph = (0, ModerationGraph_1.createModerationGraph)(this.config);
        }
        const graphContext = {
            ...context,
        ...
        const violations = violationsRaw.map((v) => typeof v === 'string'
    ...
    async _analyzeWithProvider(providerName, message, author, context) {
        if (providerName === 'lmstudio') {
            ...
            if (useStream) {
                ...
                return await provider.analyze(message, author, context, this.moderationSettings);
            }
            try {
                return await this._analyzeViaLangGraph(message, author, context);
            }
            catch (e) {
                console.warn(`LM Studio rule+model pipeline failed (${e.message}); using direct LM Studio call`);
                const provider = this.providers.lmstudio;
                return await provider.analyze(message, author, context, this.moderationSettings);
            }
        }
    ...
    constructor(config) {
        ...
        this.apiKey =
            config && config.get && config.get('ai.gemini.apiKey')
                ...
                : '';
        const modelName = (config && config.get && config.get('ai.gemini.model')) || 'gemini-2.0-flash';
        this.modelName = modelName;
        this.genAI = this.apiKey ? new generative_ai_1.GoogleGenerativeAI(this.apiKey) : null;
...
class LMStudioProvider {
    ...
    _baseUrl() {
        const raw = (this.config && this.config.get && this.config.get('ai.lmstudio.url')) ||
            'http://localhost:1234';
        return String(raw).replace(/\/$/, '');
    }
    _model() {
        return ((this.config && this.config.get && this.config.get('ai.lmstudio.model')) ||
            'local-model');
    }
    _timeout() {
        const t = (this.config && this.config.get && this.config.get('ai.lmstudio.timeout')) ||
            12000;
        return Number(t) || 12000;
    }
    _maxTokens() {
        const t = (this.config && this.config.get && this.config.get('ai.lmstudio.maxTokens')) ||
            60;
        return Number(t) || 60;
    }
    _streamingEnabled() {
        const v = this.config && this.config.get
    ...
    async analyze(message, author, context, settings) {
        ...
        const messages = [
            {
                role: 'system',
                content: 'You are a chat moderation AI. Reply with a single JSON object whose FIRST key is "isViolation" (boolean), then "severity" ("low"|"medium"|"high"), then "violations" (array), then "reasoning" (short string). No preamble, no markdown, no code fences.'
            },
        ...
        if (this._streamingEnabled()) {
            try {
                ...
                const parsed = {
                    ...this._parseResponse(text, message, author)
                };
                ...
                console.warn(`LMStudio streaming failed (${e instanceof Error ? e.message : String(e)}); falling back to non-stream`);
            }
        }
        const response = await axios_1.default.post(`${this._baseUrl()}/v1/chat/completions`, {
            model: this._model(),
            messages,
            temperature: 0,
...
class OpenAIProvider {
    ...
    async analyze(message, author, context, settings) {
        ...
        const messages = [
            {
                role: 'system',
                content: 'You are a chat moderation AI. Reply with a single JSON object whose FIRST key is "isViolation" (boolean), then "severity", "violations", "reasoning".'
            },
            { role: 'user', content: prompt }
        ];
        if (this._streamingEnabled()) {
            try {
                ...
                console.warn(`OpenAI streaming failed (${e instanceof Error ? e.message : String(e)}); falling back to non-stream`);
            }
        }
