...
// @ts-nocheck — Nested config payloads from ConfigManager/`unknown`; refine when moderation types are modeled.
const events_1 = require("events");
const crypto_1 = __importDefault(require("crypto"));
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const AIWorkerPool_1 = require("./AIWorkerPool");
 ...
 */
class ChatMonitor extends events_1.EventEmitter {
    youtubeService;
    aiService;
    config;
    isRunning;
    ...
    constructor(youtubeService, aiService, config) {
        ...
        this.messagesFile = path_1.default.join(this.storageDir, 'smart-messages.json');
        this.analysisFile = path_1.default.join(this.storageDir, 'smart-analysis.json');
        this.actionsFile = path_1.default.join(this.storageDir, 'moderation-actions.json');
    ...
    async start(options = {}) {
        ...
        try {
            ...
            const videoId = rawVideo && String(rawVideo).trim() ? String(rawVideo).trim() : null;
            if (videoId) {
                this.currentVideoId = videoId;
                this.liveChatId = await this.youtubeService.findLiveChatForVideo(videoId);
                if (!this.liveChatId) {
                    throw new Error('No live chat for that video. Use your active stream URL while live, or leave blank to use your current live broadcast.');
                }
            }
            else {
                this.currentVideoId = null;
                ...
                if (!this.liveChatId) {
                    throw new Error('No active live broadcast found. Go live on your channel, or paste a live video URL / ID.');
                }
            }
            this.isRunning = true;
            this.stats.sessionStart = new Date();
            const concurrency = Math.max(1, Number(this.config.get('ai.concurrency')) || 4);
            const queueDepth = Math.max(10, Number(this.config.get('ai.queueMaxDepth')) || 200);
            this.aiPool = new AIWorkerPool_1.AIWorkerPool({
                concurrency,
        ...
        try {
            ...
            for (const rawMessage of messages) {
                ...
                if (decision.action === 'fast') {
                    ...
                    const accepted = this.aiPool && this.aiPool.submit({ message: messageData, decision }, this._priorityScore(decision.priority));
                    if (!accepted) {
            ...
            if (messages.length > 0) {
                console.log(`Recorded ${messages.length} | Fast: ${this.fastQueue.length} | AI inflight: ${this.aiPool?.inflightCount() || 0} queued: ${this.aiPool?.size() || 0}`);
            }
        }
        catch (error) {
            if (error.code === 404) {
    ...
    _checkImmediateBlock(text, message) {
        ...
        if (userProfile && userProfile.violationCount >= 3) {
            const recentViolations = userProfile.violations.filter(v => Date.now() - v.timestamp.getTime() < 3600000 // Last hour
            );
    ...
    _calculateAIPriority(message, text) {
        let priority = 0;
        // Text-based factors
        if (this.smartRules.suspicious.some(pattern => pattern.test(text)))
            priority += 0.3;
        if (text.length > 100)
            priority += 0.1; // Longer messages need more analysis
        if (/[A-Z]{5,}/.test(message.message))
            priority += 0.2; // Caps
        ...
        const userProfile = this.userProfiles.get(message.authorId);
        if (userProfile) {
            if (userProfile.violationCount > 0)
                priority += 0.2;
            if (userProfile.messageCount < 3)
                priority += 0.1; // New users
            if (userProfile.averageMessageLength > 80)
                priority += 0.1; // Verbose users
        }
        else {
        ...
        if (this.stats.violationsFound > 0) {
            ...
            if (violationRate > 0.05)
                priority += 0.2; // High violation session
        }
    ...
    _startSmartProcessor() {
        const idleDelay = Number(this.config.get('smart.processorIdleDelayMs')) || 50;
        const busyDelay = Number(this.config.get('smart.processorBusyDelayMs')) || 0;
        const maxFast = Number(this.config.get('smart.maxFastBatchPerTick')) || 200;
        const tick = async () => {
            if (!this.isRunning)
                return;
    ...
    async _processAIItem(item) {
        ...
        try {
            ...
            const analysis = await this.aiService.analyzeMessage(message.message, message.author, {
                userContext,
    ...
    async _handleViolation(message, analysis) {
        ...
        const timeoutSec = Math.min(3600, Math.max(60, Number(this.config.get('moderation.timeoutSeconds')) || 300));
        ...
        const canApplyYouTubeMod = message.authorId &&
            !message.isModerator &&
            !message.isOwner;
        if (canApplyYouTubeMod && responseSpec.actionType === 'timeout' && autoTimeout) {
            try {
                await this.youtubeService.timeoutLiveChatUser(message.authorId, this.youtubeService.liveChatId, timeoutSec);
            ...
            try {
                await this.youtubeService.permanentlyBanLiveChatUser(message.authorId, this.youtubeService.liveChatId);
                youtubeAction = 'ban';
                youtubeActionOk = true;
            }
            catch (error) {
    ...
    async _processRemainingQueues() {
        console.log(`Processing remaining: ${this.fastQueue.length} fast, ${this.aiPool?.size() || 0} AI queued, ${this.aiPool?.inflightCount() || 0} AI inflight`);
    ...
    async _loadExistingData() {
        try {
            const messagesData = await promises_1.default.readFile(this.messagesFile, 'utf8');
            this.messageDatabase = JSON.parse(messagesData);
            const analysisData = await promises_1.default.readFile(this.analysisFile, 'utf8');
        ...
        try {
            const actionsData = await promises_1.default.readFile(this.actionsFile, 'utf8');
    ...
    async _saveData() {
        try {
            await promises_1.default.writeFile(this.messagesFile, JSON.stringify(this.messageDatabase, null, 2));
            await promises_1.default.writeFile(this.analysisFile, JSON.stringify(this.analysisDatabase, null, 2));
            await promises_1.default.writeFile(this.actionsFile, JSON.stringify(this.actionLog, null, 2));
        }
        catch (error) {
    ...
    getSmartStats() {
        ...
        const aiUsageRate = total > 0 ? Math.round((this.stats.aiProcessed / total) * 100) : 0;
        ...
        const skipped = (this.stats.smartDecisions?.immediateAllow || 0) +
        ...
        return {
            ...
            aiProcessed: this.stats.aiProcessed,
            efficiency: total > 0 ? Math.max(0, 100 - aiUsageRate) : 0,
            avgResponseTime,
            cacheHitRate,
