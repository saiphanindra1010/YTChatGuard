...
const SecretStore_1 = __importDefault(require("./SecretStore"));
const UrlAllowlist_1 = require("./UrlAllowlist");
...
function loadBuiltinYoutubeOAuth() {
    try {
        const m = require('./builtin.youtube.oauth');
...
class ConfigManager {
    configPath;
    /** Defaults only — real values come from settings.json (and optional env). */
    defaultConfig;
    /** Live merged configuration. */
    config;
    secrets;
    watchers;
    constructor() {
        ...
        this.configPath = userDataDir
            ? path_1.default.join(userDataDir, 'settings.json')
            : path_1.default.join(process.cwd(), 'src', 'config', 'settings.json');
        ...
        this.defaultConfig = {
            ...
            moderation: {
                ...
                responses: {
                    warning: '{author} ⚠️ Please keep the chat respectful. This is your warning.',
                    timeout: '{author} 🚫 Your message violated community guidelines.',
        ...
        const secretsPath = appEnv('SECRETS_PATH') ??
            path_1.default.join(path_1.default.dirname(this.configPath), 'secrets.enc');
        this.secrets = new SecretStore_1.default({ filePath: secretsPath });
    }
    /**
    ...
    async load() {
        try {
            ...
            let fileConfig = null;
            try {
                const fileContent = await promises_1.default.readFile(this.configPath, 'utf8');
            ...
            for (const p of SECRET_PATHS) {
                ...
                if (v != null)
                    this._setNestedValue(this.config, p, v);
            }
            ...
            try {
                const prev = await promises_1.default.readFile(this.configPath, 'utf8');
                if (!onDiskHasSecrets &&
                    prev.replace(/\r\n/g, '\n').trimEnd() ===
                        nextContent.replace(/\r\n/g, '\n').trimEnd()) {
                    shouldPersist = false;
    ...
    async save() {
        try {
            ...
            await promises_1.default.writeFile(this.configPath, JSON.stringify(onDisk, null, 2));
     ...
     */
    set(pathKey, value) {
        this._setNestedValue(this.config, pathKey, value);
        if (SECRET_PATHS.includes(pathKey)) {
            this.secrets.set(pathKey, typeof value === 'string' ? value : '');
            ...
            this.secrets.save().catch((err) => {
                console.error('SecretStore save failed:', err instanceof Error ? err.message : String(err));
            });
        }
    }
    /**
    ...
    async update(updates) {
        for (const [pathKey, value] of Object.entries(updates)) {
    ...
    getOAuthPublicSnapshot(listenPort) {
        ...
        return {
            ...
            listenPort: listenPort != null ? listenPort : this.get('server.port') || this.get('app.port'),
            electron: process.env.SAFESTREAM_ELECTRON === '1' ||
    ...
    getAISettingsSnapshot() {
        ...
        return {
            ...
            fallbackProviders: fallbackProviders.length > 0 ? fallbackProviders : ['gemini', 'lmstudio'],
            gemini: {
    ...
    getModerationSettingsSnapshot() {
        ...
        return {
            ...
            timeoutSeconds: Math.min(3600, Math.max(60, Number(this.get('moderation.timeoutSeconds')) || 300)),
            responses: {
    ...
    _validateConfig() {
        ...
        if (typeof aiProvider !== 'string' ||
            !['gemini', 'lmstudio', 'openai'].includes(aiProvider)) {
            errors.push(`Invalid AI provider: ${String(aiProvider)}`);
        }
        ...
        if (!this.get('youtube.clientId') || !this.get('youtube.clientSecret')) {
            console.warn('YouTube OAuth not configured: add Client ID and Secret in System → Google OAuth (or builtin.youtube.oauth.ts).');
        }
        ...
        if (aiProvider === 'lmstudio') {
            ...
            try {
                (0, UrlAllowlist_1.assertSafeLocalUrl)(typeof url === 'string' ? url : String(url ?? ''));
            }
            catch (e) {
                errors.push(`Invalid LM Studio URL: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        ...
        if (typeof strictness !== 'string' ||
            !['low', 'medium', 'high'].includes(strictness)) {
            errors.push(`Invalid moderation strictness: ${String(strictness)}`);
        }
        if (errors.length > 0) {
    ...
    _applyEnvironmentOverrides() {
        ...
        if (process.env.SAFESTREAM_ENV_OVERRIDES !== '1' &&
            process.env.YTCHATGUARD_ENV_OVERRIDES !== '1') {
            return;
        }
        // AI settings
        if (process.env.AI_PROVIDER)
            this.set('ai.provider', process.env.AI_PROVIDER);
        if (process.env.GoogleGenerativeAI)
            this.set('ai.gemini.apiKey', process.env.GoogleGenerativeAI);
        if (process.env.LMSTUDIO_URL)
            this.set('ai.lmstudio.url', process.env.LMSTUDIO_URL);
        if (process.env.LMSTUDIO_MODEL)
            this.set('ai.lmstudio.model', process.env.LMSTUDIO_MODEL);
        ...
        if (process.env.OPENAI_API_KEY)
            this.set('ai.openai.apiKey', process.env.OPENAI_API_KEY);
        // YouTube settings
        if (process.env.CLIENT_ID)
            this.set('youtube.clientId', process.env.CLIENT_ID);
        if (process.env.CLIENT_SECRET)
            this.set('youtube.clientSecret', process.env.CLIENT_SECRET);
        if (process.env.REDIRECT_URI)
            this.set('youtube.redirectUri', process.env.REDIRECT_URI);
        // Moderation settings
        ...
        if (process.env.MODERATION_TIMEOUT_SECONDS) {
            ...
            if (!Number.isNaN(s))
                this.set('moderation.timeoutSeconds', Math.min(3600, Math.max(60, s)));
        }
        console.log('SAFESTREAM_ENV_OVERRIDES=1 (or legacy YTCHATGUARD_ENV_OVERRIDES=1): full environment overrides applied');
    }
    /**
     * Deep merge objects
     */
        ...
        for (const key in source) {
            ...
            if (sk && typeof sk === 'object' && !Array.isArray(sk)) {
                const base = (target[key] && typeof target[key] === 'object' && !Array.isArray(target[key])
                    ? target[key]
                    : {});
                result[key] = this._deepMerge(base, sk);
            }
            else {
                result[key] = sk;
            }
     ...
     */
    _setNestedValue(obj, pathKey, value) {
