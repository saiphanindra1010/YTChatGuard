...
const UrlAllowlist_1 = require("./src/config/UrlAllowlist");
const projectRoot_1 = require("./src/lib/projectRoot");
const ConfigManager_1 = __importDefault(require("./src/config/ConfigManager"));
const auth_1 = __importDefault(require("./src/features/auth"));
const ChatMonitor_1 = __importDefault(require("./src/services/ChatMonitor"));
 ...
 */
class SafeStream extends events_1.EventEmitter {
    ...
    async initialize(options = {}) {
        try {
            ...
            const raw = this.config.get('server.port') ??
            ...
            await new Promise((resolve, reject) => {
                this.server = this.app.listen(port, '127.0.0.1', () => {
                    ...
                    const host = options.oauthHost ||
                        (options.electron ? '127.0.0.1' : 'localhost');
                    console.log(`SafeStream running on http://${host}:${this._listenPort}`);
    ...
    _applyRuntimeOptions(options) {
        ...
        const listenPort = this.config.get('server.port') || this.config.get('app.port') || 3000;
        if (options.electron || options.port != null) {
            this.config.set('youtube.redirectUri', `http://${host}:${listenPort}/auth/callback`);
        }
        if (options.electron) {
    ...
    _isPublicHttpRoute(req) {
        ...
        if (req.method === 'GET' && (p === '/login' || p === '/callback'))
            return true;
    ...
    _apiAuthMiddleware(req, res, next) {
        if (this._isPublicHttpRoute(req))
            return next();
    ...
    recreateYoutubeService() {
        this.youtubeService = new auth_1.default(this.config);
    ...
    recreateAIService() {
        this.aiService = new AIService_1.default(this.config);
        if (this.smartMonitor) {
     ...
     */
    _setupExpress() {
        this._apiToken = crypto_1.default.randomBytes(32).toString('base64url');
        ...
        this.app.use((req, res, next) => {
            ...
            if (origin) {
                try {
                    ...
                    const okOrigin = u.hostname === '127.0.0.1' ||
        ...
        this.app.use(express_1.default.static((0, projectRoot_1.publicDir)()));
        this._setupRoutes();
    ...
    _setupDeveloperRoutes() {
        ...
        this.app.post('/api/dev/test-ai', async (req, res) => {
            ...
            try {
                ...
                const result = await this.aiService.analyzeMessage(message || 'This is a test message', 'TestUser', { provider: provider || 'auto', testMode: true });
    ...
    _setupRoutes() {
        ...
        this.app.get('/', async (req, res) => {
            try {
                const filePath = path_1.default.join((0, projectRoot_1.publicDir)(), 'dashboard.html');
                let html = await promises_1.default.readFile(filePath, 'utf8');
        ...
        this.app.get('/api/comments/recent', (req, res) => {
            if (!this.smartMonitor?.getRecentComments)
        ...
        this.app.get('/api/events', (req, res) => {
            ...
            if (monitor) {
                if (typeof monitor.getRecentComments === 'function') {
                    for (const c of monitor.getRecentComments(50))
                        send('comment', c);
                }
            ...
            try {
                if (!this.youtubeService.isOAuthConfigured()) {
                    return res.status(503).json({
                        error: 'OAuth not configured. Open Settings and paste your Google OAuth Client ID and Secret.'
                    });
                }
        ...
        this.app.get('/login', async (req, res) => {
            try {
                if (!this.youtubeService.isOAuthConfigured()) {
                    return res
                        .status(503)
                        .send('OAuth not configured. Open Settings in the app and add your Google OAuth Client ID and Secret, then reload.');
                }
                const authUrl = await this.youtubeService.getAuthUrl();
        ...
        this.app.get('/api/ai/lmstudio/models', async (req, res) => {
            ...
            let safe;
            try {
                safe = (0, UrlAllowlist_1.assertSafeLocalUrl)(baseUrl);
            }
            catch (e) {
            ...
            try {
                const r = await axios_1.default.get(modelsPath, { timeout: 4000, maxRedirects: 0 });
        ...
        this.app.get('/api/ai/gemini/models', async (req, res) => {
            ...
            try {
                const r = await axios_1.default.get(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(key)}`, { timeout: 5000 });
        ...
        this.app.post('/api/settings/ai', async (req, res) => {
            try {
                ...
                const hasExplicitFallback = fbRaw != null && String(fbRaw).trim() !== '';
                ...
                if (b.lmstudioUrl != null) {
                    ...
                    try {
                        (0, UrlAllowlist_1.assertSafeLocalUrl)(candidate);
                    }
                    catch (e) {
        ...
        this.app.post('/api/settings/moderation', async (req, res) => {
            try {
                ...
                if (b.timeoutSeconds != null && b.timeoutSeconds !== '') {
                    ...
                    if (!Number.isNaN(s)) {
                        this.config.set('moderation.timeoutSeconds', Math.min(3600, Math.max(60, s)));
                    }
                }
                this.config._validateConfig();
                await this.config.save();
    ...
    async _initializeServices() {
        ...
        this.smartMonitor = new ChatMonitor_1.default(this.youtubeService, this.aiService, this.config);
    ...
    async shutdown() {
        ...
        if (this.isMonitoring && this.smartMonitor) {
